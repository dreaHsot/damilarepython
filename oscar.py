...
print(nom_set)
print(nom_list)
print(f'set: {len(nom_set)}')
print(f'list: {len(nom_list)}')

for n in nom_list:
    if n not in nom_count_dict.keys():
        nom_count_dict[n] = 1
    else:
        nom_count_dict[n] += 1
...
for n in nom_count_dict.values():
    sum += n
print(f'sum = {sum}')

for nom in winners.values():
    for n in nom:
        if n not in win_count_dict.keys():
            win_count_dict[n] = 1
        else:
            win_count_dict[n] += 1

print(f'win_count_dict = {win_count_dict}')
print(f'total years = {len(winners)}')
print(f'total winners = {len(win_count_dict)}')

s = 0

for n in win_count_dict.values():
    s += n

print(f'sum of wins = {s}')
...
fvalues = [f for f in freq.values()]
mn = max(fvalues)
top_winners = [f for f, n in freq.items() if n == mn]
